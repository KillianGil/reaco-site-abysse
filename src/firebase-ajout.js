...
// Fonction pour transformer vos dates textes en vrais objets Date pour Firebase
const transformerDate = (dateString) => {
    ...
    const moisAnglais = moisMap[parts[1]] || parts[1];
    return new Date(`${parts[0]} ${moisAnglais} ${parts[2]}`);
};

export async function lancerImportation() {
    console.log("🚀 Lancement de l'importation...");
    ...
    for (const art of articlesAImporter) {
        try {
            ...
            console.error(`❌ Erreur sur ${art.titre} :`, e);
        }
    }
